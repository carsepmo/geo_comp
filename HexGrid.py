...
class OHGFromPolygon(OrientedHexGrid):
    ...
    # revisar utilidad y refactorizar
    ...
    def __get_hex_centers(self):
        ...
        centers = hxt.evenq2pixel(ij_matrix.reshape(-1,2), 
                            (self._x0,self._y0), 
                            self.hex_size).reshape(n_hex_w, n_hex_h,-1)
    ...
    def get_node_neighbors(self, hex_coords):
        neighbors = hxt.get_neighbors(hex_coords)
        x_max, y_max = self.__valid_hex_tile.shape
        neighbors = neighbors[(neighbors >= 0).all(axis=1) &
                              (neighbors[:,0] < x_max) &
                              (neighbors[:,1] < y_max) &
                              self.__valid_hex_tile[neighbors[:,0],
                                                   neighbors[:,1]]]
        return neighbors
...
def test():
    ...
    plt.show()
    print(f'neighbors of (7,9):{hexgrid.get_node_neighbors((7,9))}')
