...
# modified 23-01-2024 Carlos Sepúlveda
...
# coords functions transformations
def evenq_to_cube(offsetCoord):
    if not len(offsetCoord) == 2:
        raise ValueError('offsetCoord shall be an (col, row) pair')
    if not all(isinstance(coord, int) for coord in offsetCoord):
        raise ValueError('Elements of offsetCoord shall be int, they are index!! ')
    col, row = offsetCoord
    q = col
...
def cube_to_evenq(cubeCoords):
    if not len(cubeCoords) == 3:
        raise ValueError('offsetCoord shall be an (q, r, s) triplet')
    if not all(isinstance(coord, int) for coord in cubeCoords):
...
def axial_to_evenq(axialCoord):
    if not len(axialCoord) == 2:
        raise ValueError('axialCoord shall be an (q, r) pair')
    q,r = axialCoord
...
def evenq_to_axial(evenqCoord):
    if not len(evenqCoord) == 2:
        raise ValueError('evenqCoord shall be an (col, row) pair')
    col, row = evenqCoord
    q = int(col)
...
def evenq_to_pixel_local(col:int, row:int) -> tuple:
    x = 0.5 + 3/2*col
    y = np.sqrt(3)*(0.5*((col+1)&1) +row)
    return x,y  

# Matrix for converting axial coordinates to pixel coordinates
#axial_to_pixel_mat = np.array([[SQRT3, SQRT3 / 2], [0, 3 / 2.]])
axial_to_pixel_mat = np.array([[0.5, 1.5], [0, SQRT3 / 2.]])
